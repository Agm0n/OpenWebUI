<script lang="ts">
	...
	import { config, user, tools as _tools, mobile, knowledge, chats, loadKnowledge } from '$lib/stores';
	...
	const init = async () => {
		if ($knowledge === null) {
			await knowledge.set(await loadKnowledge());
		}
	};
						...
						<div class="flex items-center w-full justify-between">
							...
							<button
								  class="bg-transparent hover:bg-gray-100 text-gray-700 dark:text-white dark:hover:bg-gray-800 rounded-full size-8 flex justify-center items-center outline-hidden focus:outline-hidden"
									on:click={async (e) => {
									e.stopPropagation();
									toast.info($i18n.t('Refreshing knowledge bases...'), {
										duration: 2000
									});
									refreshingKnowledge = true;
									loadKnowledge(true);
									
									setTimeout(() => {
										refreshingKnowledge = false;
									}, 600);
								}}
								>
								<div class={"size-4 " + (refreshingKnowledge ? 'animate-[spin_0.6s_linear_infinite]' : '')}>
									<Reset />
								</div>
							</button>
						</div>
